...
async function scrape(symbol: string) {
    ...
    // const response = await axios(`https://www.google.com/search?q=${symbol}`)
    const response = await axios(`https://www.google.com/finance/quote/${symbol}-USD`)
    const html = response.data;

    const $ = cheerio.load(html);
    const value = $('.YMlKec.fxKbKc').html() ?? '0'

    // const value = $('.pclqee').html()
    const symbolValue = {
        symbol,
        value,
        when: new Date()
...
async function work() {
    ...
    // console.log('scraped ', result, 'symbols')
